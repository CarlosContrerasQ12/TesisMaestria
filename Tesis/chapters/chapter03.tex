...
\begin{equation}
	...
	\end{split},
\end{equation}
where we use the notation for the norms $\norm{f(y)}_{\mathcal{Y},\nu}^2=\int_{\mathcal{Y}}|f(y)|^2d\nu$, given a positive probability density on $\mathcal{Y}$. Here, $J(\varphi)$ measure how well $\phi$ satisfies the PDE with its initial and boundary conditions. If $J(\varphi)=0$, then $\varphi$ is a solution to \eqref{eqn:PDEBoundaryCh2}.

To obtain the minimizing parameters $\theta$, we will perform a training procedure of the neural network with a stochastic gradient descent algorithm. We will sample points according to $v_1,v_2,v_3,v_4$ and calculate a Monte-Carlo approximation of the functional loss \eqref{eqn:DGMLoss} using automatic differentiation to calculate the derivatives appearing in the PDE operator and boundary conditions. With this approximation, we will take a descent step in the gradient direction calculated again using automatic differentiation. 
